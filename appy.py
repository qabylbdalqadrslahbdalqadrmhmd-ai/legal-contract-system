...
.analysis-card:hover { border-color: #e94560; }

.strength-card {
    border-right: 4px solid #00d4aa;
    background: linear-gradient(135deg, #0d2b25, #0f2d27);
}
.weakness-card {
    border-right: 4px solid #e94560;
    background: linear-gradient(135deg, #2b0d1a, #2d0f1e);
}
.suggestion-card {
    border-right: 4px solid #f7c948;
    background: linear-gradient(135deg, #2b2500, #2d2600);
}
.missing-card {
    border-right: 4px solid #ff6b35;
    background: linear-gradient(135deg, #2b1500, #2d1600);
}
...
<div class="main-header">
    ...
    <p>تحليل شامل واحترافي للعقود القانونية بالذكاء الاصطناعي</p>
</div>
""", unsafe_allow_html=True)
...
# Helper: Call Claude
# ─────────────────────────────────────────────
def analyze_contract(client: anthropic.Anthropic, contract_text: str,
    ...
    system_prompt = """أنت محامٍ وخبير قانوني متخصص في تحليل العقود، تتحدث العربية بطلاقة.
مهمتك تحليل العقود وتقديم تقرير شامل واحترافي باللغة العربية فقط.
أجب دائماً بـ JSON صحيح فقط بدون أي نص خارج الـ JSON."""

    user_prompt = f"""حلل هذا العقد وأعطني JSON بالتنسيق التالي بالضبط:
...
{{
  ...
  "missing_clauses": [
    {{"title": "البند الناقص", "importance": "عالي/متوسط/منخفض", "suggestion": "نص مقترح للبند"}}
  ],
  "risks": [
    {{"title": "الخطر", "severity": "عالي/متوسط/منخفض", "mitigation": "طريقة التخفيف"}}
  ],
  "improvements": [
    {{"title": "التحسين المقترح", "detail": "شرح التحسين", "priority": "عالي/متوسط/منخفض"}}
  ],
  "score": {{
    "overall": 75,
    "clarity": 80,
  ...
  "verdict": "الحكم النهائي: هل العقد جيد للتوقيع، يحتاج تعديلات، أم يجب رفضه",
  "recommendation": "توصية مفصلة للشخص قبل التوقيع"
}}

نوع العقد المتوقع: {contract_type_hint}
مستوى التفصيل: {detail}

خيارات التحليل المطلوبة:
...
def generate_contract(client: anthropic.Anthropic, contract_type: str,
                      details: str, lang: str = "ar") -> str:
    system_prompt = "أنت محامٍ خبير في صياغة العقود القانونية باللغة العربية. اكتب عقوداً احترافية وشاملة ومتوازنة."
    user_prompt = f"""اكتب عقد {contract_type} احترافي وشامل باللغة العربية.
...
with tab1:
    ...
    if input_method == "📝 نص مباشر":
        ...
        if pdf_file:
            ...
            if PYMUPDF_AVAILABLE:
                ...
                if contract_text:
                    ...
                    st.warning("⚠️ لم يُستخرج نص — سيُرسل PDF كصورة للذكاء الاصطناعي")
                    image_data_dict = {"media_type": "application/pdf",
                                       "data": file_to_base64(file_bytes)}
            else:
                st.info("📤 سيُرسل PDF مباشرة للذكاء الاصطناعي")
                image_data_dict = {"media_type": "application/pdf",
                                   "data": file_to_base64(file_bytes)}
        ...
        if docx_file:
            ...
            if DOCX_AVAILABLE:
                ...
                st.error("❌ مكتبة python-docx غير مثبتة. أضف `python-docx` لـ requirements.txt")
    ...
    if can_analyze:
        ...
        if st.button("🔍 حلل العقد الآن", disabled=not api_key):
            if not api_key:
                st.error("❌ أدخل API Key أولاً من الشريط الجانبي")
            else:
                with st.spinner("🤖 الذكاء الاصطناعي يحلل العقد... قد يستغرق دقيقة"):
                    try:
...
<div class="analysis-card weakness-card">
    ...
    <p style="color:#c0c0d8">{item.get('detail','')}</p>
        ...
        with dt3:
            for item in r.get("missing_clauses", []):
...
<div class="analysis-card missing-card">
    <div class="card-title missing-title">🔍 {item.get('title','')} 
        <span style="font-size:0.8rem;color:{imp_color}">({imp})</span>
    </div>
    <p style="color:#c0c0d8"><strong style="color:#ff6b35">نص مقترح:</strong> {item.get('suggestion','')}</p>
</div>""", unsafe_allow_html=True)

        with dt4:
            for item in r.get("risks", []):
...
<div class="analysis-card" style="border-right:4px solid {sev_color}">
    <div class="card-title" style="color:{sev_color}">⚠️ {item.get('title','')}
        <span style="font-size:0.8rem">— خطورة: {sev}</span>
    </div>
...
<div class="analysis-card suggestion-card">
    <div class="card-title suggestion-title">💡 {item.get('title','')}
        <span style="font-size:0.8rem;color:{pri_color}">— أولوية: {pri}</span>
    </div>
    <p style="color:#c0c0d8">{item.get('detail','')}</p>
</div>""", unsafe_allow_html=True)
...
with tab2:
    ...
    gen_type = st.selectbox("نوع العقد المطلوب", [
        ...
        "عقد مقاولة", "عقد نقل ملكية", "عقد قرض", "آخر (حدد التفاصيل)",
    ])
    
    gen_details = st.text_area(
        "تفاصيل العقد",
        height=200,
        placeholder="""مثال:
الطرف الأول: شركة ABC للتجارة
الطرف الثاني: محمد أحمد
الراتب: 8000 جنيه شهرياً
مدة العقد: سنة قابلة للتجديد
الإجازة: 21 يوم سنوياً
..."""
    )
    
    if st.button("🪄 أنشئ العقد الآن", disabled=not api_key):
        if not api_key:
            st.error("❌ أدخل API Key أولاً")
        elif not gen_details.strip():
            st.warning("⚠️ أدخل تفاصيل العقد")
        else:
            with st.spinner("✍️ الذكاء الاصطناعي يكتب العقد..."):
                try:
                    client = anthropic.Anthropic(api_key=api_key)
                    generated = generate_contract(client, gen_type, gen_details)
    ...
    if "generated_contract" in st.session_state:
        ...
        st.markdown(f"""
<div class="contract-output">{st.session_state['generated_contract']}</div>
""", unsafe_allow_html=True)
        ...
        with col_b:
            if st.button("🔍 حلل هذا العقد"):
                st.session_state["analyze_generated"] = True
                st.info("💡 انتقل لتبويب 'تحليل عقد موجود' والصق النص")
...
with tab3:
    ...
    clauses = {
        "البنود الأساسية في كل عقد": {
            "color": "#00d4aa",
            "items": [
                ("تعريف الأطراف", "الاسم الكامل، العنوان، رقم الهوية/السجل التجاري لكل طرف"),
...
# Footer
st.markdown("""
<div style="text-align:center;padding:2rem;color:#404060;font-size:0.85rem;margin-top:3rem;
            border-top:1px solid #2a2a4a">
    ⚖️ محلل العقود الذكي | مدعوم بـ Claude AI من Anthropic<br>
